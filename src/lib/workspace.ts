...
function emptySettings(): WorkspaceSettings {
  return {
    ...
    research_webhook_url: null,
    research_auto_add: true,
...
export function integrationStatus(s: WorkspaceSettings): IntegrationStatus {
  return {
    ...
    slack: Boolean(s.slack_bot_token && s.slack_signing_secret),
...
export function redactSettings(s: WorkspaceSettings) {
  ...
  return {
    ...
    slack_bot_token: mask(s.slack_bot_token),
    slack_signing_secret: mask(s.slack_signing_secret),
